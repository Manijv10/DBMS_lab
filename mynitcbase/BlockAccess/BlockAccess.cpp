...
//Relation catalog
  //first we reset the search index and perform blockaccess::linearsearch(relcat_relid,relcatrelname,newname) to check the new relation name already exists.If so return E_RELEXISTS.
 //else we check the recid of the old relation name.If it is (-1,-1) no such relation exists hence return  e_relnotexists.
 //Once u get the req recid,using getrecord and attribute we store the required record in the array.Then perform strcpy to chnage the relation name to the new name.
 //finally perform setrecord(record,recId.slot) to update the new changes

 //Attribute catalog
 //repeat same steps for attribute catalog by iteration through ATTRCAT_NO _ATTRS
...
int BlockAccess::renameRelation(char oldName[ATTR_SIZE],char newName[ATTR_SIZE]) {
  ...
  RecId relcatRecId = BlockAccess::linearSearch(RELCAT_RELID, RELCAT_ATTR_RELNAME,
                                          oldRelationName, EQ);
  ...
  relcatRecId = BlockAccess::linearSearch(
...
//Then we move on to locating the attribute that we need to rename in that relation.For that first locate the relation name in the attrcatalog.
 //if we get a valid recId then check if attrname=newattrname.Therefore it already exists hence return.Else locate the oldattrname=attrname and change it to new name.
